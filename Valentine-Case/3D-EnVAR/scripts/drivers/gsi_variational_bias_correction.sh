...
while [ ${dmn} -le ${max_dom} ]; do
  ...
  while [ ${bc_loop} -le ${MAX_BC_LOOP} ]; do
    ...
    if [ ${bc_loop} -ne ${MAX_BC_LOOOP} ]; then
      # create storage for the outputs given the bias correction loop except for final loop
      bc_dir=bc_loop_${bc_loop}
      mkdir ${bc_dir}
      cd ${bc_dir}
    fi
    ...
    if [ bc_loop -e 0 ]; then
      ...
      tar_file=${obs_root}/abiaspc.${anl_date}.tar.gz
      if [ -r "${tar_file}" ]; then
        cd ${obs_root}
        tar -xvf `basename ${tar_file}`
        bias_file=${obs_root}/${anl_date}.abiaspc/gdas.abiaspc.t${hh}z.${anl_date}.txt
        if [ -r "${bias_file}" ]; then
          echo "Link the GDAS combined bias correction file for loop zero of analysis"
          cd ${work_dir}
          ln -sf ${bias_file} ./satbias_in
        else
          echo "GDAS combined bias correction file not readable at ${bias_file}"
          exit 1
        fi
      else
    ...
    # begin next loop from the workdir
    cd ${workdir}
  done 
