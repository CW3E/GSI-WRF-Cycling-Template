...
# ENS_SIZE      = INT   : Size of the ensemble, not including the 00 control forecast
...
if [[ ${IF_HYBRID} = ${YES} ]] ; then
  ...
  if [ ! "${ENS_SIZE}" ]; then
    echo "ERROR: \$ENS_SIZE must be specified to the number of ensemble perturbations!"
    exit 1
  fi
fi
...
while [ ${dmn} -le ${MAX_DOM} ]; do
  ...
  if [[ ${IF_COLD_START} = ${YES} ]]; then
    BKG_FILE=${INPUT_DATAROOT}/realprd/ens_00/wrfinput_d0${dmn}
  else
    BKG_FILE=${WRFDA_ROOT}/lower_bdy_update/ens_00/wrfout_d0${dmn}_${DATE_STR}
  fi
  ...
  if [[ ${IF_HYBRID} = ${YES} ]] ; then
    ifhyb=.true.
    ${ECHO} " GSI hybrid uses n_ens=${ENS_SIZE}"
  ...
  if [[ ${IF_HYBRID} = ${YES} ]]; then
    ...
    while [ ${ens_n} -le ${ENS_SIZE} ]; do
      # two zero padding for GEFS
      iimem=`printf -v ${ens_n} "%02d"`

      # three zero padding for GSI
      iiimem=`printf -v ${ens_n} "%03d"`

      if [[ ${IF_COLD_START} = ${YES} ]]; then
        ENS_FILE=${INPUT_DATAROOT}/realprd/ens_${iimem}/wrfinput_d0${dmn}
      else
        ENS_FILE=${WRFDA_ROOT}/lower_bdy_update/ens_${iimem}/wrfout_d0${dmn}_${DATE_STR}
      fi
  ...
  if [[ ${IF_OBSERVER} = ${YES} ]]; then
    ...
    while [[ ${ens_n} -le ${ENS_SIZE} ]]; do
       ...
       # run GSI
       echo ' Run GSI with' ${bk_core} 'for member ', ${iiimem}
