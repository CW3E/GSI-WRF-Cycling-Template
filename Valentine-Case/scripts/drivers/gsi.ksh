...
#####################################################
# Below are defined depending on the value of MAX_DOM with a while loop producing
# analyses of multiple domains
#
...
while [ ${dmn} -le ${MAX_DOM} ]; do
  ...
  # make copies of fort files with domain and date name
  set -A GSI_FORT_FILES ./fort.*
  for file in ${GSI_FORT_FILES[@]}; do
    ${ECHO} "${CP} -sf ${file}"
    ${CP} ${file} ./${file}.d0${dmn}_${ANAL_TIME}
  done
