...
#SBATCH --job-name="sio3d_realexe"
#SBATCH --nodes=4
#SBATCH --ntasks-per-node=24
#SBATCH -t 01:30:00
#SBATCH -o "3dvar_out.o"
#SBATCH --partition=compute
#SBATCH -A cwp102
#SBATCH --export=ALL
#SBATCH --mail-type=ALL
#SBATCH --mail-user=mzheng@ucsd.edu
...
#export NETCDF='/opt/netcdf/3.6.2/intel'
...
# GSI_EXE  = path and name of the gsi executable 
  ANAL_TIME=2019021300
  HH=`echo $ANAL_TIME | cut -c9-10`
#  HOME_ROOT=/home/minghua/gsi/gsi3.6/comGSIv3.6_EnKFv1.2/dtc
  HOME_ROOT=/home/minghua/Work_cwp102/scratch/minghua/GSI_WORKFLOW_TEST/3DVar
  WORK_ROOT=${HOME_ROOT}/work/2019AR_IOP3_${ANAL_TIME}_only3dvarWRF
  OBS_ROOT=${HOME_ROOT}/data/${ANAL_TIME}
  PREPBUFR=${OBS_ROOT}/gdas.t${HH}z.prepbufr.nr
  BK_ROOT=${HOME_ROOT}/arw/${ANAL_TIME}
  BK_FILE=${BK_ROOT}/wrfout_d01_2019-02-13_00:00:00
...
# Set up namelist
  ...
  GSI_NAMELIST=${HOME_ROOT}/nml/namelist_3dvar.DROP
  ymdz1=2019021218
#
#------------------------------------------------
...
# setup for GSI 3D/4D EnVar hybrid
  if [ ${if_hybrid} = Yes ] ; then
    ENS_ROOT=${HOME_ROOT}/data/2016AR/${ANAL_TIME}
#    ENSEMBLE_FILE_mem=${ENS_ROOT}/2018012818/sfg_2014061700_fhr06s
    ENSEMBLE_FILE_mem=${ENS_ROOT}/${ymdz1}/wrf

    if [ ${if_4DEnVar} = Yes ] ; then
      BK_FILE_P1=${BK_ROOT}/wrfout_d01_2016-02-14_03:00:00
      BK_FILE_M1=${BK_ROOT}/wrfout_d01_2016-02-13_21:00:00

      #ENSEMBLE_FILE_mem_p1=${ENS_ROOT}/2018012818/sfg_2014061700_fhr09s
      #ENSEMBLE_FILE_mem_m1=${ENS_ROOT}/2018012818/sfg_2014061700_fhr03s
      ENSEMBLE_FILE_mem_p1=${ENS_ROOT}/${ymdz1}/wrf_f15s
      ENSEMBLE_FILE_mem_m1=${ENS_ROOT}/${ymdz1}/wrf_f09s
    fi
  fi
...
# ln -s ${OBS_ROOT}/gdas.t${HH}z.sptrmm.tm00.bufr_d tmirrbufr
...
srcobsfile[9]=${OBS_ROOT}/gdas.t${HH}z.mtiasi.tm00.bufr_d
...
srcobsfile[22]=${OBS_ROOT}/gdas.t${HH}z.goesfv.tm00.bufr_d
gsiobsfile[22]=gsnd1bufr
ii=1
while [[ $ii -le 22 ]]; do
...
#  ls ${ENSEMBLE_FILE_mem}_mem* > filelist02
  ls ${ENSEMBLE_FILE_mem}_en* > filelist06
  if [ ${if_4DEnVar} = Yes ] ; then
    ls ${ENSEMBLE_FILE_mem_p1}_en* > filelist03
    ls ${ENSEMBLE_FILE_mem_m1}_en* > filelist01
  fi
  
  nummem=`more filelist06 | wc -l`
  nummem=$((nummem -3 ))
...
#CONVINFO=${FIX_ROOT}/global_convinfo_ALL_IPW_SAT.txt
CONVINFO=${FIX_ROOT}/global_convinfo_NoDROP.txt
...
cp ${OBS_ROOT}/gdas.abias.t00z.20160214.txt ./satbias_in
cp ${OBS_ROOT}/gdas.abiaspc.t00z.20160214.txt ./satbias_pc
cp ${OBS_ROOT}/gdas.t00z.abias ./satbias_in
cp ${OBS_ROOT}/gdas.t00z.abias_pc ./satbias_pc
...
mpirun_rsh -export -np ${GSIPROC}  -hostfile $SLURM_NODEFILE ./gsi.exe >stdout 2>&1;;
esac
